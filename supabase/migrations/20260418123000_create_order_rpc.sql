-- Atomically create order + line items from published products (server-side prices).
-- Allows returning notify_token to guests without permissive anon SELECT on orders.
-- Uses scalar variables only (avoids %ROWTYPE issues in some SQL runners).
...
begin
  ...
  values (
    ...
    case when auth.uid() is null then lower(trim(p_guest_email)) else null end,
  ...
  returning id, notify_token into v_order_id, v_notify;
