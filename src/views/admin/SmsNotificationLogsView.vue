...
const functionOptions = computed(() => {
  const unique = new Set(rows.value.map((r) => r.function_name).filter(Boolean));
...
const filteredRows = computed(() => {
  ...
  return rows.value.filter((r) => {
    ...
    if (functionFilter.value !== "all" && r.function_name !== functionFilter.value) return false;
    if (!q) return true;
    return (
...
const sentCount = computed(() => rows.value.filter((r) => r.status === "sent").length);
...
<template>
  <div>
    ...
    <div class="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
      <div class="rounded-2xl border border-emerald-200/80 bg-emerald-50/70 px-4 py-3">
        <p class="text-[11px] font-semibold uppercase tracking-wide text-emerald-800">Sent</p>
        <p class="mt-1 text-xl font-bold tabular-nums text-emerald-900">{{ sentCount }}</p>
      </div>
      <div class="rounded-2xl border border-rose-200/80 bg-rose-50/70 px-4 py-3">
        <p class="text-[11px] font-semibold uppercase tracking-wide text-rose-800">Failed</p>
        <p class="mt-1 text-xl font-bold tabular-nums text-rose-900">{{ failedCount }}</p>
      </div>
      <div class="rounded-2xl border border-amber-200/80 bg-amber-50/70 px-4 py-3">
        <p class="text-[11px] font-semibold uppercase tracking-wide text-amber-800">Skipped</p>
        <p class="mt-1 text-xl font-bold tabular-nums text-amber-900">{{ skippedCount }}</p>
      </div>
    </div>

    <div class="mt-6 rounded-2xl border border-zinc-200 bg-white p-4 sm:p-5">
      <div class="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,11rem)_minmax(0,16rem)_minmax(0,1fr)]">
        <select
        ...
        >
          <option v-for="opt in functionOptions" :key="`fn-${opt}`" :value="opt">
    ...
    >
      <div class="border-b border-zinc-200 bg-zinc-50 px-4 py-2.5 text-xs font-semibold text-zinc-500 sm:px-5">
        Showing {{ filteredRows.length }} of {{ rows.length }} logs
      </div>

      <div v-if="loading" class="px-5 py-8 text-sm text-zinc-500">Loading logs...</div>
      <div v-else-if="!filteredRows.length" class="px-5 py-8 text-sm text-zinc-500">
        No SMS logs match your current filters.
      </div>
      <div v-else class="max-h-[64dvh] overflow-auto">
        <table class="min-w-full border-collapse text-left text-sm">
          <thead class="sticky top-0 z-[1] bg-zinc-100/95 text-[11px] uppercase tracking-wider text-zinc-500">
            <tr>
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-3">
                <code class="rounded bg-zinc-100 px-1.5 py-0.5 text-[11px] text-zinc-700">
                  {{ row.function_name }}
                </code>
              </td>
              ...
              <td class="px-4 py-3 text-xs text-zinc-600">{{ row.detail || "—" }}</td>
            </tr>
          </tbody>
        </table>
      </div>
